...
// WithRLPGatewayClientErrorStream returns a RLPGatewayClientOption to
// configure a channel to which errors concerning gateway connection will be
// sent. It defaults to nil.  If the supplied channel is full and nothing is
// listening on it, the client will block attempting to reconnect until the
// error is pulled off the channel.
